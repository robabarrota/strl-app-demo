...
const statHeaders = [{ key: 'points', label: 'POINTS' }];
...
const MedalCount = () => {
	...
	const {
		content: medalCount,
		loading: medalCountLoading,
		error: medalCountError,
		fetched: medalCountFetched,
	} = useSelector(getMedalCount);
	const formatDriverName = useFormatDriverName();

	if (!medalCountFetched && !medalCountLoading && !medalCountError)
		dispatch(fetchMedalCount());
	...
	useEffect(() => {
		...
		if (sortBy === null) {
			const newSortedMedalCount = [
				...medalCountCopy.sort((a, b) =>
					tableSortFunction(a, b, defaultSortBy)
				),
			];
			setSortedMedalCount(newSortedMedalCount);
		} else if (sortBy.key === 'driver') {
			const sortedRaceResults = [
				...medalCountCopy.sort((a, b) => nameSortFunction(a, b, sortBy)),
			];
			setSortedMedalCount(sortedRaceResults);
		} else {
			const sortedRaceResults = [
				...medalCountCopy.sort((a, b) => tableSortFunction(a, b, sortBy)),
			];
			setSortedMedalCount(sortedRaceResults);
		}
	}, [medalCount, sortBy]);
					...
					<thead>
						<tr>
							{medalHeaders.map((header) => (
								...
								>
									<i className={`${header.icon} ${bem(header.key)}`}></i>{' '}
		...
		return (
			<g transform={`translate(${x},${y})`}>
				<text x={0} y={0} dy={5} fill={'#666'} textAnchor="end">
					<tspan>{formatDriverName(payload.value)}</tspan>
				</text>
			</g>
		);
	};
