...
const Schedule = () => {
	...
	const scrollToNextRace = () => {
		if (nextRaceRef.current) {
			...
			const yOffset = -76;
			const y =
				nextRaceRef.current.getBoundingClientRect().top +
				window.pageYOffset +
				yOffset;
	...
	const isDataReady = useMemo(
		() => trackListFetched && !trackListLoading,
		[trackListFetched, trackListLoading]
	);
	...
	const futureRaces = useMemo(() => {
		...
		return trackList.filter(
			({ date, key }) => new Date(date) > now && key !== nextTrack.key
		);
	}, [trackList, nextTrack]);
								...
								<div className={bem('month-container')}>
									...
									{isRaceOver(date) && (
										<span className={bem('finish-banner-container')}>
											<img
												src="https://www.formula1.com/etc/designs/fom-website/images/flag-asset.png"
												alt="race finished"
											/>
										</span>
									)}
				...
				{nextTrack && (
					<div className={bem('current-race-container')} ref={nextRaceRef}>
						<CurrentEventCard key={`round-${completedRaces.length + 1}`}>
							<RoundTitle>
								Round {completedRaces.length + 1} - Up Next
							</RoundTitle>
							<CurrentRaceDetailsContainer $isMobile={isMobile}>
								<div className={bem('current-race-track-panel')}>
									<div className={bem('event-info')}>
										<div className={bem('top-bar')}>
											<div>
												<p className={bem('days')}>
													{getDayRange(nextTrack.date)}
												</p>
												<div className={bem('month-container')}>
													<span className={bem('month')}>
														{new Date(nextTrack.date).toLocaleString(
															'default',
															{ month: 'short' }
														)}
													</span>
											...
											<CountryFlag>
												<img
													src={trackDetails[nextTrack.label]?.flag}
													alt={`${nextTrack.label} flag`}
												/>
											</CountryFlag>
										</div>
									</div>
									...
									<div className={bem('current-event-image')}>
										<img
											src={trackDetails[nextTrack.label]?.whiteMap}
											alt={`${nextTrack.label} map`}
										/>
									</div>
								</div>
