...
const statHeaders = [
	...
	{
		key: 'fastestLaps',
		label: <i className="fa-solid fa-stopwatch race-results__fastest-icon"></i>,
	},
];
...
const RaceResults = () => {
	...
	const {
		content: fastestLaps,
		loading: fastestLapsLoading,
		fetched: fastestLapsFetched,
		error: fastestLapsError,
	} = useSelector(getFastestLaps);
	const {
		content: trackList,
	...
	useEffect(() => {
		...
		if (sortBy === null) {
			const newSortedStats = [
				...statsCopy.sort((a, b) =>
					tableSortFunction(a, b, defaultSortBy, ['averageFinish'])
				),
			];
			setSortedStats(newSortedStats);
			const sortedDrivers = newSortedStats.map((stat) => stat.driver);
			setSortedRaceResults([
				...raceResultsCopy.sort(
					(a, b) =>
			...
			const newSortedStats = [
				...statsCopy.sort((a, b) =>
					tableSortFunction(a, b, sortBy, ['averageFinish'])
				),
			];
			setSortedStats(newSortedStats);
			const sortedDrivers = newSortedStats.map((stat) => stat.driver);
			...
			const newSortedRaceResults = [
				...raceResultsCopy.sort((a, b) =>
					tableSortFunction(a, b, sortBy, 'all')
				),
			];
			setSortedRaceResults(newSortedRaceResults);
			const sortedDrivers = newSortedRaceResults.map(
	...
	const renderResultsSubTable = useMemo(() => {
		const fastestLapClass = (driverName, track) => {
			if (fastestLaps[track] === driverName && fastestLaps[track] !== undefined)
				return bem('fastest');
		};
		return (
