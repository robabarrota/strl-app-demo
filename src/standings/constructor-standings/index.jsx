...
const ConstructorStandings = ({ show }) => {
	...
	const {
		...
		fetched: raceResultsFetched,
		error: raceResultsError,
	} = useSelector(getRaceResults);
	const {
		content: constructorPoints,
		loading: constructorPointsLoading,
		fetched: constructorPointsFetched,
		error: constructorPointsError,
	} = useSelector(getConstructorPoints);
	const {
		content: constructorStats,
		loading: constructorStatsLoading,
		fetched: constructorStatsFetched,
		error: constructorStatsError,
	} = useSelector(getConstructorStats);
	const {
		content: trackList,
	...
	if (
		!constructorPointsFetched &&
		!constructorPointsLoading &&
		!constructorPointsError
	)
		dispatch(fetchConstructorPoints());
	if (
		!constructorStatsFetched &&
		!constructorStatsLoading &&
		!constructorStatsError
	)
		dispatch(fetchConstructorStats());
	...
	const constructors = useMemo(
		() => [...new Set(participants?.map(({ car }) => car))],
		[participants]
	);
	...
	useEffect(() => {
		...
		if (sortBy === null) {
			const newSortedStats = [
				...statsCopy.sort((a, b) => tableSortFunction(a, b, defaultSortBy)),
			];
			setSortedStats(newSortedStats);
			const sortedConstructors = newSortedStats.map((stat) => stat.car);
			setSortedConstructorPoints([
				...constructorPointsCopy.sort(
					(a, b) =>
						sortedConstructors.indexOf(a.car) -
						sortedConstructors.indexOf(b.car)
				),
			]);
		} else if (
			...
			setSortedStats(newSortedStats);
			const sortedDrivers = newSortedStats.map((stat) => stat.car);
			setSortedConstructorPoints([
				...constructorPointsCopy.sort(
					(a, b) => sortedDrivers.indexOf(a.car) - sortedDrivers.indexOf(b.car)
				),
			]);
		} else {
			const sortedConstructorPointsCopy = [
				...constructorPointsCopy.sort((a, b) =>
					tableSortFunction(a, b, sortBy)
				),
			];
			setSortedConstructorPoints(sortedConstructorPointsCopy);
			const sortedConstructors = sortedConstructorPointsCopy.map(
				(raceResult) => raceResult.car
			);
			setSortedStats([
				...statsCopy.sort(
					(a, b) =>
		...
		() =>
			trackList.reduce((acc, track) => {
				...
				constructorPoints.forEach((row) => {
					...
					if (result === 'DNS' || result === 'DNF' || result === undefined)
						return;
						...
						{sortedConstructorPoints.map(({ car }) => (
							<tr key={car}>
								<td className={bem('table-cell')}>
									...
									>
										{formatConstructorName(car)}{' '}
										<ConstructorBadge constructor={car} />
									</TableTooltip>
								</td>
							</tr>
						))}
			...
			<table>
				...
				<tbody>
					{sortedStats.map((stats) => (
						<tr key={stats.car}>
							<td className={bem('table-cell')}>{stats.total}</td>
							<td className={bem('table-cell')}>
								<TableTooltip
									innerHtml={round(stats.averagePoints, {
										decimalPlace: 8,
									})}
									hangLeft
								>
									{round(stats.averagePoints)}
								</TableTooltip>
							</td>
						</tr>
					))}
