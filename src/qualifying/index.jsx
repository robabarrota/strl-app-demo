...
const Qualifying = () => {
	...
	const {
		content: qualifyingResults,
		loading: qualifyingLoading,
		error: qualifyingResultsError,
	} = useSelector(getQualifying);
	const {
		content: trackList,
	...
	if (
		isEmpty(qualifyingResults) &&
		!qualifyingLoading &&
		!qualifyingResultsError
	)
	...
	if (isEmpty(driverStats) && !driverStatsLoading && !driverStatsError)
	...
	useEffect(() => {
		...
		if (sortBy === null) {
			const newSortedStats = [
				...statsCopy.sort((a, b) =>
					tableSortFunction(a, b, defaultSortBy, ['averageQualifying'])
				),
			];
			setSortedStats(newSortedStats);
			const sortedDrivers = newSortedStats.map((stat) => stat.driver);
			setSortedQualifyingResults([
				...qualifyingResultsCopy.sort(
					(a, b) =>
			...
			const newSortedStats = [
				...statsCopy.sort((a, b) =>
					tableSortFunction(a, b, sortBy, ['averageQualifying'])
				),
			];
			setSortedStats(newSortedStats);
			const sortedDrivers = newSortedStats.map((stat) => stat.driver);
			...
			const newSortedQualifyingResults = [
				...qualifyingResultsCopy.sort((a, b) =>
					tableSortFunction(a, b, sortBy, 'all')
				),
			];
			setSortedQualifyingResults(newSortedQualifyingResults);
			const sortedDrivers = newSortedQualifyingResults.map(
				(qualifyingResult) => qualifyingResult.driver
			);
			setSortedStats([
				...statsCopy.sort(
					(a, b) =>
	...
	const isDataReady =
		!isEmpty(sortedQualifyingResults) &&
		!qualifyingLoading &&
		!isEmpty(trackList) &&
		!trackListLoading &&
