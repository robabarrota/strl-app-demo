...
const Header = () => {
	...
	const clickedOutsideAdminMenu = useIsOutsideClick(userMenuContainerRef, (res) => adminMenuOpen && setAdminMenuOpen(!res));
...

					{
						isLoggedIn ? (
							<div className={bem('user-nav-container')}>
								<button className={bem('user-nav-button')} onClick={() => setAdminMenuOpen(curr => !curr)}>
									<span className={bem('nav-link-text')}>{activeUser.username?.[0].toUpperCase()}</span>
								</button>
								{
									adminMenuOpen && (
										<div className={bem('admin-menu')} ref={userMenuContainerRef} onBlur={() => setAdminMenuOpen(false)}>
											<NavLink className={bem('admin-nav')} to="/admin/seasons" onClick={closeHeader}>
												{() => (
											...
											<NavLink className={bem('admin-nav')} to="/admin/account" onClick={closeHeader}>
												{() => (
													<div className={bem('admin-link-label')}>
														<span className={bem('nav-link-text')}>Account Settings</span>
														<i className={"fa-solid fa-chevron-right"}></i>
													</div>
												)}
											</NavLink>
											<button className={bem('admin-nav')} onClick={() => dispatch(logout())}>
												{
													<div className={bem('admin-link-label')}>
							...
							<div className={bem('admin-nav-container')}>
								<NavLink className={bem('admin-nav')} to="/admin/seasons" onClick={closeHeader}>
									{() => (
										<span className={bem('nav-link-text')}><i className="fa-solid fa-gear"></i></span>
									)}
								</NavLink>
							</div>
