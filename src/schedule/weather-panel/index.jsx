...
const Stint = styled.div`
	background: linear-gradient(
		149deg
			${(props) =>
				props.$gradientStages?.map(
					(stage, index) =>
						`, ${stage} ${(index / props.$gradientStages.length) * 100}%`
				)}
	);
	flex: 1;
`;
...
const WeatherItem = styled.div`
	${(props) =>
		!props.$isFirst ? 'border-left: 1px solid rgba(208,208,210,.4);' : ''};
	display: flex;
...
const WeatherPanel = ({ trackInfo }) => {
	...
	const [tbdWeatherIcon, setTbdWeatherIcon] = useState({
		iconIndex: 0,
		src: weatherIcons[0],
	});
	...
	useEffect(() => {
		const timer1 = setTimeout(() => {
			if (isTbdIconTransitioning) {
				const nextIconIndex =
					(tbdWeatherIcon?.iconIndex || 0 + 1) % weatherIcons.length;
		...
		isTbdIconTransitioning,
		setIsTbdIconTransitioning,
		tbdWeatherIcon?.iconIndex,
	]);
			...
			<Stint $gradientStages={qualifyingWeatherGradientStages}>
				...
				<WeatherRow>
					<WeatherItem $isMobile={isMobile} $isFirst={true}>
						...
						<WeatherIcon
							$transitioning={
								!weatherIconMap[trackInfo?.q1Weather] && isTbdIconTransitioning
							}
							src={weatherIconMap[trackInfo?.q1Weather] || tbdWeatherIcon.src}
						/>
					</WeatherItem>
					<WeatherItem $isMobile={isMobile}>
						...
						<WeatherIcon
							$transitioning={
								!weatherIconMap[trackInfo?.q2Weather] && isTbdIconTransitioning
							}
							src={weatherIconMap[trackInfo?.q2Weather] || tbdWeatherIcon.src}
						/>
					</WeatherItem>
				</WeatherRow>
			</Stint>
			<Stint $gradientStages={raceWeatherGradientStages}>
				...
				<WeatherRow>
					<WeatherItem $isMobile={isMobile} $isFirst={true}>
						...
						<WeatherIcon
							$transitioning={
								!weatherIconMap[trackInfo?.r1Weather] && isTbdIconTransitioning
							}
							src={weatherIconMap[trackInfo?.r1Weather] || tbdWeatherIcon.src}
						/>
					</WeatherItem>
					<WeatherItem $isMobile={isMobile}>
						...
						<WeatherIcon
							$transitioning={
								!weatherIconMap[trackInfo?.r2Weather] && isTbdIconTransitioning
							}
							src={weatherIconMap[trackInfo?.r2Weather] || tbdWeatherIcon.src}
						/>
					</WeatherItem>
					<WeatherItem $isMobile={isMobile}>
						...
						<WeatherIcon
							$transitioning={
								!weatherIconMap[trackInfo?.r3Weather] && isTbdIconTransitioning
							}
							src={weatherIconMap[trackInfo?.r3Weather] || tbdWeatherIcon.src}
						/>
					</WeatherItem>
				</WeatherRow>
			</Stint>
