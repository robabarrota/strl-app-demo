...
const DriverStandings = ({ show }) => {
	...
	const {
		content: driverPoints,
		loading: driverPointsLoading,
		fetched: driverPointsFetched,
		error: driverPointsError,
	} = useSelector(getDriverPoints);
	const {
		content: trackList,
	...
	if (!driverPointsFetched && !driverPointsLoading && !driverPointsError)
		dispatch(fetchDriverPoints());
	...
	useEffect(() => {
		if (driverPoints.length && driverStats.length) {
			...
			if (sortBy === null) {
				...
				setSortedDriverPoints([
					...driverPointsCopy.sort(
						(a, b) =>
				...
				const newSortedDriverPoints = [
					...driverPointsCopy.sort((a, b) => tableSortFunction(a, b, sortBy)),
				];
				setSortedDriverPoints(newSortedDriverPoints);
				const sortedDrivers = newSortedDriverPoints.map(
					(raceResult) => raceResult.driver
				);
				setSortedDriverStats([
					...statsCopy.sort(
						(a, b) =>
							sortedDrivers.indexOf(a.driver) - sortedDrivers.indexOf(b.driver)
					),
						...
						{sortedDriverPoints.map((row) => (
							<tr key={row.driver} className={bem('row')}>
								{trackList.map(({ key, label }, index) => (
									...
									>
										<TableTooltip innerHtml={label}>{row[key]}</TableTooltip>
									</td>
								))}
							</tr>
						))}
